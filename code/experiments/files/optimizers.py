...
class GDOptimizer:
    def __init__(self, gradient_approximator, learning_rate_k, x_0, sett=None, x_sol=None, max_oracle_calls=10**4, tol=0.000001, seed=18):
        np.random.seed(seed)
        ...
        if x_sol is not None:
            if self.func_name == "quadratic": 
                self.f_sol = utils.quadratic_func(self.x_sol, A=self.A, b=self.b, c=self.c)
            elif self.func_name == "logreg":
                self.f_sol = utils.logreg_func(self.x_sol, X=self.X, y=self.y, alpha=self.alpha)
    ...
    def get_error(self, x):
        if self.x_sol is None:
            if self.func_name == "quadratic":  #||f(x_k)||
                error = utils.quadratic_func(x, A=self.A, b=self.b, c=self.c)
            elif self.func_name == "logreg":  #||f(x_k)||
                error = utils.logreg_func(x, X=self.X, y=self.y, alpha=self.alpha)
            elif self.func_name == "SVM":  #||f(x_k)||
...
class FWOptimizer(GDOptimizer):
    def __init__(self, gradient_approximator, learning_rate_k, 
        ...
        super().__init__(gradient_approximator, learning_rate_k, x_0, sett, x_sol, max_oracle_calls, tol, seed)
        self.name = "FW"
